...
class ExtraChargeDeleteView(FeatureAccessMixin, DeleteView):
    ...
    # A charge created from a health record (vaccination, farrier, vet,
    # worming) is regenerated by sync_record_charge from record.cost on the
    # record's next save — deleting only the charge silently resurrects it
    # (e.g. a disputed fee reappearing when someone fixes a typo in the
    # record's notes). Zero the source record's cost too, so the deletion
    # sticks.
    ...
    def form_valid(self, form):
        record = self._linked_health_record(self.object)
        if record is not None:
            record.cost = Decimal('0.00')
            record.extra_charge = None
            record.save(update_fields=['cost', 'extra_charge'])
            messages.success(
                self.request,
                "Charge deleted. The linked health record's cost was reset "
                "to £0.00 so the charge won't be recreated.",
            )
        else:
            messages.success(self.request, "Charge deleted successfully.")
        return super().form_valid(form)

    def _linked_health_record(self, charge):
        from django.core.exceptions import ObjectDoesNotExist
        for accessor in self.HEALTH_RECORD_ACCESSORS:
            try:
                # Reverse one-to-one raises DoesNotExist (not AttributeError),
                # so getattr's default can't be used here.
                return getattr(charge, accessor)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                # Record a YardCost only for manually-entered costs. A cost
                # derived from purchased stock was already recorded as a
                # YardCost when the delivery was logged — creating another
                # here double-counted every bale in cost reporting (~£1,500
                # reported for £500 actually spent once fed out + recharged).
                if not cost_from_stock and feed_out.total_cost > 0:
                ...
                # Recharge to horse owners. Only horses with an owner share
                # the cost — an owner-less horse in the selection used to
            ...
            if skipped_ownerless:
                messages.warning(
                    request,
                    "Not recharged (no owner to bill): "
