...
>
> **Update (mobile batch):** Part 4 items **O1 (tables→cards on 14 templates), O2 residuals, O3 (ownership-page checkboxes), O4 (bulk-select hit areas), O5 (tab-strip scroll fade), O6 (paired date grids), N1 (PWA: manifest, icons, favicon, theme-color), N2 (lazy Chart.js), N4–N8 (cramming/touch-target/inputmode fixes)** are ✅ done — verified at 390px with Playwright (zero horizontal overflow on 12 pages, Chart.js absent on non-chart pages, manifest served). `/placements/` turns out to redirect to the locations history tab; the orphaned `PlacementListView` template was converted anyway. Still open from Part 4: N3 (image thumbnails), N10 (self-hosted fonts — needs font assets), and the strategic items (quick-add sheet, offline queue, web push).
>
