...
>
> **Update (workflow batch):** workflow findings **#1 (bulk send / bulk mark-paid), #4 (health records return to the horse page, plus "Save & add another"), #5 (live invoice preview wired up), #6 (charge form derives the owner from the horse, with a server-side mismatch warning), and #7 ("Cost per horse" labelling on bulk actions)** are ✅ fixed, with tests in `horse_management/invoicing/test_workflow_batch.py`. Remaining open: Parts 3–4 (see batch updates below).
>
> **Update (mobile batch):** Part 4 items **O1 (tables→cards on 14 templates), O2 residuals, O3 (ownership-page checkboxes), O4 (bulk-select hit areas), O5 (tab-strip scroll fade), O6 (paired date grids), N1 (PWA: manifest, icons, favicon, theme-color), N2 (lazy Chart.js), N4–N8 (cramming/touch-target/inputmode fixes)** are ✅ done — verified at 390px with Playwright (zero horizontal overflow on 12 pages, Chart.js absent on non-chart pages, manifest served). `/placements/` turns out to redirect to the locations history tab; the orphaned `PlacementListView` template was converted anyway. Still open from Part 4: N10 (self-hosted fonts — needs font assets) and the strategic items (quick-add sheet, offline queue, web push).
>
> **Update (thumbnails batch):** Part 4 item **N3 (image thumbnails)** is ✅ done: `Horse.photo_thumb` — a 320px square JPEG rendition generated on save (regenerated on photo change, cleared on removal, failure-tolerant) — is what all avatars now render, with explicit width/height to prevent layout shift and fallback to the original until `python manage.py generate_thumbnails` backfills pre-existing photos. A 30-horse list now downloads ~10-20 KB per avatar instead of multi-MB originals. Tests in `horse_management/core/tests/test_thumbnails.py`.
>
> **Update (documents batch):** feature gap **#5 (documents/attachments)** is ✅ implemented: a `Document` model (passport / insurance / registration / loan agreement / vet report / other) attachable to a horse or an owner, with file validation, optional expiry dates, a Documents card on the horse and owner detail pages (staff upload/delete, camera-friendly on mobile), and a daily reminder task emailing the yard a summary of documents expiring within 30 days (one reminder per document, re-armed if the expiry date changes; claim-then-rollback like the other reminder tasks). Media already requires login (bug #11 fix). Tests in `horse_management/core/tests/test_documents.py`.
>
> **Update (finance-completion batch):** feature gaps **#3 (VAT) and #4 (aged debtors & owner statements)** are ✅ implemented. VAT: a `vat_rate` business setting (validated to 0 or 20), snapshotted per invoice so history never rewrites, computed into `vat_amount`/`total`, shown on the preview/detail/PDF, and driving the Xero CSV + API tax codes — the PDF-says-£0-while-Xero-adds-20% inconsistency is gone (net lines + Xero VAT now always equal the invoice total). Debtors: `/invoicing/debtors/` buckets every open balance (part-payments deducted) into current/1–30/31–60/61–90/90+, linked from the Finances Outstanding KPI. Statements: per-owner statement page with Invoiced/Paid/Balance KPIs, PDF download, and staff Email-with-attachment, linked from owner detail and each debtors row. Tests in `horse_management/invoicing/test_statements_vat.py`.
>
> **Update (quality-of-life batch):** workflow findings **#8, #9, and the #10 nits** are ✅ fixed: dashboard Vaccinations/Farrier Due widgets get one-click "Record" actions (returning to the dashboard) and their "View all" links point at the tabbed health dashboard; the Outstanding Invoices widget gets Record Payment / Mark Paid row actions; the invoice list gains a period filter with whole-result-set totals (invoiced + outstanding, part-payments deducted) sharing filter semantics with the CSV export; creating a horse lands on its detail page; Quick Find now surfaces departed horses (labelled, sorted after active ones). This closes **all of Part 2**. Tests in `horse_management/invoicing/test_qol.py`.
>
> **Update (payments batch):** workflow finding **#3 / feature gap #1 (payment recording)** is ✅ implemented: a `Payment` ledger (date, amount, method, reference) with partial-payment support, a Record Payment flow on the invoice, running Paid-to-Date/Balance-Due on the invoice detail, list, dashboard, PDF, and overdue reminder emails; one-click and bulk Mark-as-Paid record balancing payments so the ledger always reconciles; the Xero paid-status sync records a Xero payment; deleting a payment reverts the status. Tests in `horse_management/invoicing/test_payments.py`. Follow-ons unblocked: aged debtors / owner statements (gap #4) and automated Xero payment sync (gap #2).
>
> **Update (automation batch):** feature gaps **#2 (automated Xero sync) and #6 (auto-generation of monthly invoices)** are ✅ implemented: a nightly Celery task sweeps every pushed, still-open invoice and marks Xero-paid ones paid locally (with a "Paid in Xero" ledger payment, so overdue reminders stop chasing settled invoices); a beat task on the 1st creates draft invoices for the month just ended (duplicate-safe, drafts only, toggleable in Settings via `auto_generate_invoices`); and the invoice bulk bar gains **Push to Xero**. Schedule times configurable via `XERO_SYNC_HOUR` / `MONTHLY_INVOICE_HOUR` env vars. Tests in `horse_management/invoicing/test_automation.py`.
